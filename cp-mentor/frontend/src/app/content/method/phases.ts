...
// ── Compact copy deck (2026-08-04) ────────────────────────────────────────
// Terse, imperative rewrite of the prose above, rendered by default. The
// original prose fields above are unchanged and rendered as `full` text
// behind a "Why this works" expander. See CLAUDE.md's "Copy Deck" note.

export const MINDSET_COMPACT: Record<'TWO_ARTEFACTS' | 'MODEL_COMPUTATION_NOT_PHYSICS', string> = {
  TWO_ARTEFACTS:
    'Produce two things:\n1. A working brute force.\n2. One sentence stating why it is too slow.\n\n' +
    'Both present → the attempt succeeded.',
  MODEL_COMPUTATION_NOT_PHYSICS:
    'Model the computation, not the physics. Not "how does water flow" — "what number do I ' +
    'compute at each index."'
};

export const RESTATE_COMPACT = {
  instruction: 'Restate in one sentence, your own words. Can\'t → you misread it.',
  badExample: 'Find the water.',
  goodExample: 'For each index, compute how much water sits above that bar, sum them.',
  bruteForce: 'State a brute force out loud, however absurd. Never be at zero.',
  diagnostic: 'Can\'t state one → wrong unit of analysis → go straight to Phase 2.'
};

export const HAND_SOLVE_COMPACT = 'n=5, on paper, then ask what your brain did.';

export const CODE_COMPACT = 'No clear plan → do not type. Meaningful names.';

export interface JavaHabitRow {
  do: string;
  not: string;
}

export const JAVA_HABITS_TABLE: JavaHabitRow[] = [
  { do: 'lo + (hi - lo) / 2', not: '(lo + hi) / 2 — overflows' },
  { do: 'long for sums past 2³¹', not: 'int accumulators' },
  { do: 'leftMax, writeIndex, windowStart', not: 'a, temp, x' },
  { do: 'int[] in hot loops', not: 'List<Integer> — autoboxing' },
  { do: 'map.getOrDefault(k, 0), map.merge(k, 1, Integer::sum)', not: 'manual null checks' },
  { do: 'Deque<Integer> s = new ArrayDeque<>()', not: 'Stack — legacy, synchronised' }
];

export const SORT_NOTE_COMPACT =
  'Arrays.sort(): int[] → dual-pivot quicksort, O(n²) worst case. Integer[] → stable TimSort.';

export const DRY_RUN_COMPACT = 'n=1, all same, all negative, sorted, duplicates, overflow';

export const TIME_CAP_COMPACT = 'Set an actual timer. Past the cap you are rehearsing being stuck.';
